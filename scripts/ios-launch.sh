...
BUNDLE_ID="com.huely.app"

if ! command -v xcrun >/dev/null 2>&1 || ! xcrun devicectl --version >/dev/null 2>&1; then
  echo "devicectl is unavailable. Open Huely manually on your iPhone."
  exit 1
fi
...
if ! xcrun devicectl list devices --json-output "$json_file" >/dev/null 2>&1; then
  echo "Could not list connected devices. Open Huely manually on your iPhone."
  exit 1
fi
...
if [[ -z "$udid" ]]; then
  echo "No connected iPhone found. Plug in your device or open Huely manually."
  exit 1
fi
